...
def make_inputs(batch: int = 3) -> tuple[np.ndarray, np.ndarray]:
...
def main(argv: list[str] | None = None) -> int:
    args = finalize_args(make_parser("RISC-V dynamic batched gemm example").parse_args(argv))
    func, rt_mod = build_riscv_module(dynamic_batched_gemm, "dynamic_batched_gemm")

    maybe_print_tir(func, args.print_tir)
    maybe_emit_artifacts(rt_mod, "dynamic_batched_gemm", args)

    if args.run_host:
        lhs, rhs = make_inputs()
        out = np.zeros((lhs.shape[0], M, N), dtype=np.float32)
        run_host(rt_mod, "dynamic_batched_gemm", lhs, rhs, out, output_dir=args.output_dir)
    ...
    if args.run_qemu:
        lhs, rhs = make_inputs()
        out = np.zeros((lhs.shape[0], M, N), dtype=np.float32)
        run_qemu(rt_mod, "dynamic_batched_gemm", lhs, rhs, out, output_dir=args.output_dir)
        np.testing.assert_allclose(out, lhs @ rhs)
